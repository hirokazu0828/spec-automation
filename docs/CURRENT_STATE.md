...
| `src/components/Step2/index.test.tsx` *(Layer 6 UX fix)* | 15 | Route A 自動提案実行の 7 条件分岐 (originRoute / proposals 既存 / headShape・position 空) + GuideBanner の 8 表示パターン (A 表示 / B 表示+lookup / B fallback / C 非表示 / undefined 非表示 / 閉じるボタン) |
