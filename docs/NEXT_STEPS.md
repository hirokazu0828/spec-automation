# Layer 0 〜 Layer 3a 完了 → Phase B (マレット線画化) / Layer 5 / Layer 3b・3c / Layer A への申し送り

> 直近完了レイヤ:
> - Layer 0 〜 Layer 2-PDF-perf: PR #1〜#9 → 統合ブランチ → PR #10 で `main` にマージ済
> - Layer 3a (型カタログ + アングル選択 UI): `claude/layer3a-template-catalog` (本 PR)
...
1. サンプル帳 (`src/data/samples.json`) からセミマレット 9 件・フルマレット 27 件のうち寸法・confidence 高い代表を 1 件ずつ選定
...
5. `metadata.source` / `createdAt` / `logoTreatment` を埋める
6. `helpers.test.ts` の `getCompleteTemplates` テストを 1 件 → 3 件に更新
